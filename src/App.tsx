...
function App(): JSX.Element {
    ...
    function getMessages(messages: Message[]): JSX.Element[] {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            mes.push(<div key={i} className={'chat'}>
                <h1 className="profile fa-solid fa-circle-user">&nbsp;</h1>
                <div className={'flex flex-direction-column'}>
                    <div className={'flex flex-direction-row'}>
                        <h4>{sender}</h4> <span className={'timestamp'}>&nbsp;{timestamp}</span>
                    </div>
                    <span>{text}</span>
                </div>
            </div>)
    ...
    function onChatEnter(event: React.KeyboardEvent<HTMLInputElement>): void {
        if (event.key === 'Enter') {
            ...
            setInput('');
            setChat([{text: input, sender: name, timestamp: new Date().toTimeString()}, ...chat]);
            socket.emit('message', {text: input, sender: name, timestamp: new Date().toTimeString()});
        }
    }

    return (
        <div className={'container'}>
            {name === '' ? <div className={'name_container'}>
                <input
                ...
                />
                <button className={'name_input'} onClick={
                    () => {
                        if (room === '') return;
                        socket.emit('join', inputName, room);
                        setName(inputName);
                ...
                }>Click to join the chat room</button>
            </div> : <>
                <input
                    type="text"
                    className={'width-50'}
